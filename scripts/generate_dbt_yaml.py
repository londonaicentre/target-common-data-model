...
# dependencies = ["linkml-runtime==1.11.1", "pyyaml==6.0.3"]
# ///
...
def build_column(sv: SchemaView, cls_name: str, slot_name: str, enums, classes) -> dict:
    ...
    for key in ("fhir_path", "fhir_type", "value_set"):
        ...
        if val is not None:
            meta[key] = val
    if is_variant:
        variant_slots = sv.class_induced_slots(rng)
        pk = next((s.name for s in variant_slots if s.identifier), None)
        meta["variant_class"] = rng
        if pk:
            meta["variant_pk"] = pk
        meta["variant_fields"] = [s.name for s in variant_slots if not s.identifier]
    if meta:
